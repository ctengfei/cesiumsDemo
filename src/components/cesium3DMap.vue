...
let pointLightTest = null; //光源
let pointEntity = null; //光源实体
let layerS3M = null;//获取坝体bim模型
let layerS3Mqx = null;//获取坝体倾斜摄影
export default {
    name: 'testMap',
    components: {
        arxRender
    ...
    mounted() {
        ...
        this.$bus.on('skyChange', val => {
            ...
            this.$bus.on('ObliquePhotography', val => {
                this.ObliquePhotography(val);
            });
        // 淹没效果
        this.$bus.on('floodChange', val => {
            ...
            this.$bus.on('PointLight', val => {
                if (val) {
                    ...
                    scene.removeLightSource(pointLightTest);
    ...
    methods: {
        ...
        loadMap() {
            ...
            document.getElementsByClassName('cesium-viewer-timelineContainer')[0].setAttribute('style', 'display: none;');
            // var scene = window.viewer.scene;
            var hl = scene.open('http://36.135.21.38:10151/iserver/services/3D-FuShuiSanWeiChangJing/rest/realspace', 'fssk', {
                autoSetView: false
            });
            var ditu = scene.open('http://36.135.21.38:10151/iserver/services/3D-fushuiyx/rest/realspace');
            hl.then(layer => {
                layer.map(m => {
                    if (m._name == 'fushuishuiku_adjust@DataSourceNew') {
                        layerS3M = m;
                    }
                    if(m._name =='btqx'){
                        layerS3Mqx = m;
        ...
        ObliquePhotography(val) {
            //倾斜摄影坝体显示
            layerS3M.visible = !val;
            layerS3Mqx.visible = val;
        },
        flyRoute() {
        ...
        PointLight() {
            ...
            pointLightTest = new Cesium.PointLight(position, options);
            scene.addLightSource(pointLightTest);
        },
